---
exchange: gateio
source_url: https://www.gate.com/docs/developers/options/ws/en
api_type: WebSocket
updated_at: 2026-05-29 19:59:05.156072
---

# Gate Options WebSocket v4.0.0
...
Gate provides a simple and robust Websocket API to integrate Gate options underlying trade status into your business or application.
...
2026.05-12
  ...
  * Multiple channels support underlying-scoped subscription: `subscribe` / `unsubscribe` may use a JSON object as `payload` with `underlyings` and optional `contracts`. The server expands each underlying to all listed options contracts for that underlying, unions them with explicit `contracts`, and de-duplicates.
  * Affected channels: `options.contract_tickers`, `options.contract_candlesticks`, `options.book_ticker`, `options.trades`, `options.mark_prices`, `options.contracts`, `options.settlements`, `options.orders`, `options.usertrades`, `options.positions`, `options.liquidates`, `options.position_closes`, `options.user_settlements`.
  * Per-request limits apply to the number of `underlyings` and to the total expanded contract count; when exceeded, `error.code` is `2` and `message` typically contains `too many`.

2025.12-18

  * `options.orders` update descriptions for push parameters

2025.11-19

  * `options.positions` update client subscription instructions
...
`payload` | Array / Object | Yes | request detail parameters; some channels also accept an object for underlying-scoped subscription, see "Underlying-scoped subscription" below and each channel  
...
###  Underlying-scoped subscription (object payload)

On supported channels, `subscribe` / `unsubscribe` may use a JSON object as `payload` in addition to the string array form. Combine channel-specific fields (if any) with the common fields below.

Underlying-scoped subscription (object payload)Field | Type | Required | Description  
---|---|---|---  
`underlyings` | `Array[String]` | conditional | Option underlying names (e.g. `BTC_USDT`). Must be supported by the server. At least one of `underlyings` and `contracts` must be non-empty.  
`contracts` | `Array[String]` | conditional | Explicit options contract names. Unioned with contracts expanded from each underlying, then de-duplicated.  
  
**Public channels:** `options.contract_tickers`, `options.contract_candlesticks`, `options.book_ticker`, `options.trades`, `options.mark_prices`, `options.contracts`, `options.settlements`.

**Private channels (auth required):** `options.orders`, `options.usertrades`, `options.positions`, `options.liquidates`, `options.position_closes`, `options.user_settlements`.

Per-request limits apply to the number of `underlyings` and to the total expanded contract count. When exceeded, `error` is non-null with `code` `2` and `message` typically containing `too many`.

The legacy private array form such as `["<userId>", "!all"]` remains supported and can coexist with the object form on the same connection as separate subscription scopes.
...
####  Object payload (underlying scope)

Besides the string array above, `payload` may be a JSON object. In addition to the common `underlyings` / `contracts` fields described under **Underlying-scoped subscription** above, this channel supports:

Object payload (underlying scope)Field | Type | Required | Description  
---|---|---|---  
`change_from` | String | No | Statistics window, e.g. `utc0`, `24h`  
...
This channel sends a trade message whenever a trade occurs at Gate. It includes details of the trade, such as price, amount, time.
...
Besides the string array above, `payload` may be a JSON object using the common `underlyings` / `contracts` fields (at least one non-empty), as described under **Underlying-scoped subscription** above.
...
This channel sends all trades message under underlying whenever a trade occurs at Gate. It includes details of the trade, such as price, amount, time. **This channel doesn't push underlying trade data, but all contract trade data under the specified underlying**
...
Besides the string array above, `payload` may be a JSON object. `interval` is **required** , combined with the common `underlyings` / `contracts` fields (at least one non-empty) described under **Underlying-scoped subscription** above.
...
How to maintain local order book:
  ...
  3. Retrieve base order book using REST API, and make sure the order book ID is recorded(referred as `baseID` below) e.g. `https://api.gateio.ws/api/v4/options/order_book?contract=BTC_USDT-20211130-50000-C&limit=10&with_id=true` retrieves the 10-level base order book of BTC_USDT
...
You can find example application implementing the methods above in the [SDK GitHub repository ](https://github.com/gateio/gatews)
...
Besides the `[userId, contract, ...]` array above, `payload` may be a JSON object. **Authentication** is required (see **Authentication** above). Use the common `underlyings` / `contracts` fields (at least one non-empty) from **Underlying-scoped subscription**. This can coexist on the same connection with forms such as `["<userId>", "!all"]`.
...
Besides the string array above, `payload` may be a JSON object. **Authentication** is required. Use the common `underlyings` / `contracts` fields (at least one non-empty) from **Underlying-scoped subscription**. This can coexist with `!all` array subscriptions on the same connection.
...
Besides the string array above, `payload` may be a JSON object. **Authentication** is required. Use the common `underlyings` / `contracts` fields (at least one non-empty) from **Underlying-scoped subscription** above.
...
If you want to subscribe to all position updates, use `!all` in the list.
...
---|---|---|---  
`user id` | String | Yes | User id  
`contract` | String | Yes | Options contract name, `!all` \- indicates subscribing to all option contracts  
