...
api_type: WebSocket
updated_at: 2026-06-17 18:50:13.900667
---
...
Use the code samples to subscribe to one or more Advanced Trade WebSocket channels using CDP API Keys.
  ...
  * Python
      ...
      const jwt = sign(
        {
          iss: "cdp",
          nbf: Math.floor(Date.now() / 1000),
          exp: Math.floor(Date  ws.send(JSON.stringify(subscribeMsg));
    }
    ...
    def on_message(ws, message):
        data = json.loads(message)
        with open("Output1.txt", "a")                 sent_unsub = True
                time.sleep(1)
