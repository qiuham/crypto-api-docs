...
api_type: WebSocket
updated_at: 2026-06-15 19:20:59.163901
---
...
> Code Snippet
    ...
    public class Main {
        public static void main(String[] args) throws Exception {
            ...
            String name        'nonce' => $nonce  // Nonce included in headers for added security
        ];
        $jwtToken = JWT::encode($jwtPayload, $privateKeyResource, 'ES256', $keyName, $headers);
        return $jwtToken;
    }
...
> Code Snippet
    ...
    public class Main {
        public static void main(String[] args) throws Exception {
            ...
            JWSSigner signer = newInstant.now().getEpochSecond() + 120);
            data.put("sub", name);
...
> Code Snippet
    ...
    #include <iostream>
    #include ECDSASigner(ecPrivateKey);
            signedJWT.sign(signer);
...
nbf: Math.floor(Date.now() / 1000), exp: Math.floor(Date.now() / 1000) + 120, sub: keyName, }; const header = { alg: algorithm, kid: keyName, nonce: crypto.randomBytes(16).toString('hex'), }; return jwt.sign(payload, keySecret, { algorithm, header }); }; const main = () => { const token = generateJWT(); console.log(token); }; main(); `
