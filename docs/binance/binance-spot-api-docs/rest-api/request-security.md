...
api_type: REST
updated_at: 2026-01-15T23:36:19.428343
---
...
* 每个接口都有一个鉴权类型，指示所需的 API 密钥权限，显示在接口名称旁边（例如，[下新订单 (TRADE)](/docs/zh-CN/binance-spot-api-docs/rest-api/market-data-endpoints#place-new-order-trade)）。
  * 如果未指定，则鉴权类型为 `NONE`。
  * 除了为 `NONE` 外，所有具有鉴权类型的接口均视为 `SIGNED` 请求（即包含 `signature`），[listenKey 管理](/docs/zh-CN/binance-spot-api-docs/rest-api/request-security#user-data-stream-requests) 除外。
...
### 需要签名的接口[​](/docs/zh-CN/binance-spot-api-docs/rest-api/request-security#需要签名的接口 "需要签名的接口的直接链接")

  * 调用`SIGNED` 接口时，除了接口本身所需的参数外，还需要在 `query string` 或 `request body` 中传递 `signature`, 即签名参数。



#### 签名是否是大小写敏感的[​](/docs/zh-CN/binance-spot-api-docs/rest-api/request-security#签名是否是大小写敏感的 "签名是否是大小写敏感的的直接链接")
...
请参阅[已签名请求示例 (HMAC)](/docs/zh-CN/binance-spot-api-docs/rest-api/request-security#hmac-keys)、[已签名请求示例 (RSA)](/docs/zh-CN/binance-spot-api-docs/rest-api/request-security#rsa-keys) 和[已签名请求示例 (Ed25519)](/docs/zh-CN/binance-spot-api-docs/rest-api/request-security#ed25519-keys)，了解如何根据您使用的 API 密钥类型计算签名。

### 时间同步安全[​](/docs/zh-CN/binance-spot-api-docs/rest-api/request-security#时间同步安全 "时间同步安全的直接链接")

  * `SIGNED` 请求还需要一个 `timestamp` 参数，该参数应为当前时间戳，单位为毫秒或微秒。（参见 [通用 API 信息](/docs/zh-CN/binance-spot-api-docs/rest-api/autogen_intro#general-api-information)）
...
### POST /api/v3/order 的签名示例[​](/docs/zh-CN/binance-spot-api-docs/rest-api/request-security#post-apiv3order-的签名示例 "POST /api/v3/order 的签名示例的直接链接")

#### HMAC Keys[​](/docs/zh-CN/binance-spot-api-docs/rest-api/request-security#hmac-keys "HMAC Keys的直接链接")
...
#### RSA Keys[​](/docs/zh-CN/binance-spot-api-docs/rest-api/request-security#rsa-keys "RSA Keys的直接链接")
...
#### Ed25519 Keys[​](/docs/zh-CN/binance-spot-api-docs/rest-api/request-security#ed25519-keys "Ed25519 Keys的直接链接")
