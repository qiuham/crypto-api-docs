---
exchange: binance
source_url: https://developers.binance.com/docs/binance-spot-api-docs/testnet/fix-api
api_type: REST
updated_at: 2026-06-17 19:01:13.607035
---
...
## General API Information[​](/docs/binance-spot-api-docs/testnet/fix-api#general-api-information "Direct link to General API Information")
...
### NodeJS[​](/docs/binance-spot-api-docs/testnet/fix-api#nodejs "Direct link to NodeJS")
...
### Other languages/custom TLS implementations[​](/docs/binance-spot-api-docs/testnet/fix-api#other-languagescustom-tls-implementations "Direct link to Other languages/custom TLS implementations")
...
### FIX API Order Entry sessions[​](/docs/binance-spot-api-docs/testnet/fix-api#fix-api-order-entry-sessions "Direct link to FIX API Order Entry sessions")
  ...
  * Supports receiving all of the account's [ExecutionReport`<8>`](/docs/binance-spot-api-docs/testnet/fix-api#executionreport) and [List Status`<N>`](/docs/binance-spot-api-docs/testnet/fix-api#liststatus).
...
### FIX API Drop Copy sessions[​](/docs/binance-spot-api-docs/testnet/fix-api#fix-api-drop-copy-sessions "Direct link to FIX API Drop Copy sessions")
...
### FIX API Market Data sessions[​](/docs/binance-spot-api-docs/testnet/fix-api#fix-api-market-data-sessions "Direct link to FIX API Market Data sessions")
...
### FIX Connection Lifecycle[​](/docs/binance-spot-api-docs/testnet/fix-api#fix-connection-lifecycle "Direct link to FIX Connection Lifecycle")
  ...
  * There is no minimum connection time guarantee; a server can enter maintenance at any time. 
    * When a server enters maintenance, a [News `<B>`](/docs/binance-spot-api-docs/testnet/fix-api#news) message will be sent to clients **every 10 seconds until disconnection** , prompting clients to reconnect. Upon receiving this message, a client is expected to establish a new session and close the old one. If the client does not close the old session before the server disconnects it, the server will proceed to log it out and close the session.
  * After connecting, the client must send a Logon `<A>` request. For more information please refer to [How to sign a Logon request](/docs/binance-spot-api-docs/testnet/fix-api#signaturecomputation).
  ...
  * The system allows negotiation of the `HeartInt (108)` value during the logon process. Accepted values range between 5 and 60 seconds. 
    * If the server has not sent any messages within a `HeartInt (108)` interval, a [HeartBeat `<0>`](/docs/binance-spot-api-docs/testnet/fix-api#heartbeat) will be sent.
    * If the server has not received any messages within a `HeartInt (108)` interval, a [TestRequest `<1>`](/docs/binance-spot-api-docs/testnet/fix-api#testrequest) will be sent. If the server does not receive a HeartBeat `<0>` containing the expected `TestReqID (112)` from the client within `HeartInt (108)` seconds, the server will send a Logout `<5>` message and close the connection.
...
### API Key Permissions[​](/docs/binance-spot-api-docs/testnet/fix-api#api-key-permissions "Direct link to API Key Permissions")
...
### On message processing order[​](/docs/binance-spot-api-docs/testnet/fix-api#on-message-processing-order "Direct link to On message processing order")

The `MessageHandling (25035)` field required in the initial [Logon`<A>`](/docs/binance-spot-api-docs/testnet/fix-api#logon-request) message controls whether messages from the client may be reordered before they are processed by the Matching Engine.
...
### Response Mode[​](/docs/binance-spot-api-docs/testnet/fix-api#response-mode "Direct link to Response Mode")

By default, all concurrent order entry sessions receive all of the account's successful [ExecutionReport`<8>`](/docs/binance-spot-api-docs/testnet/fix-api#executionreport) and [ListStatus`<N>`](/docs/binance-spot-api-docs/testnet/fix-api#liststatus) messages, including those in response to orders placed from other FIX sessions and via non-FIX APIs.

Use the `ResponseMode (25036)` field in the initial [Logon`<A>`](/docs/binance-spot-api-docs/testnet/fix-api#logon-request) message to change this behavior.
...
### Timing Security[​](/docs/binance-spot-api-docs/testnet/fix-api#timing-security "Direct link to Timing Security")
...
### How to sign Logon `<A>` request[​](/docs/binance-spot-api-docs/testnet/fix-api#how-to-sign-logon-a-request "Direct link to how-to-sign-logon-a-request")

The [Logon`<A>`](/docs/binance-spot-api-docs/testnet/fix-api#logon-main) message authenticates your connection to the FIX API. This must be the first message sent by the client.
...
## Limits[​](/docs/binance-spot-api-docs/testnet/fix-api#limits "Direct link to Limits")

### Message Limits[​](/docs/binance-spot-api-docs/testnet/fix-api#message-limits "Direct link to Message Limits")
  ...
  * Breaching the message limit results in immediate [Logout `<5>`](/docs/binance-spot-api-docs/testnet/fix-api#logout) and disconnection.
  * To understand current limits and usage, please send a [LimitQuery`<XLQ>`](/docs/binance-spot-api-docs/testnet/fix-api#limitquery) message. A [LimitResponse`<XLR>`](/docs/binance-spot-api-docs/testnet/fix-api#limitresponse) message will be sent in response, containing information about Order Rate Limits and Message Limits.
...
### Connection Limits[​](/docs/binance-spot-api-docs/testnet/fix-api#connection-limits "Direct link to Connection Limits")
  ...
  * Upon breaching the limit a [Reject `<3>`](/docs/binance-spot-api-docs/testnet/fix-api#reject) will be sent containing information about the connection limit breach and the current limit.
...
### Unfilled Order Count[​](/docs/binance-spot-api-docs/testnet/fix-api#unfilled-order-count "Direct link to Unfilled Order Count")

  * To understand how many orders you have placed within a certain time interval, please send a [LimitQuery`<XLQ>`](/docs/binance-spot-api-docs/testnet/fix-api#limitquery) message. A [LimitResponse`<XLR>`](/docs/binance-spot-api-docs/testnet/fix-api#limitresponse) message will be sent in response, containing information about Unfilled Order Count and Message Limits.
...
## Error Handling[​](/docs/binance-spot-api-docs/testnet/fix-api#error-handling "Direct link to Error Handling")

Client messages that contain syntax errors, missing required fields, or refer to unknown symbols will be rejected by the server with a [Reject `<3>`](/docs/binance-spot-api-docs/testnet/fix-api#reject) message.
...
The list of error codes can be found on the [Error codes](/docs/binance-spot-api-docs/testnet/errors) page.

## Types[​](/docs/binance-spot-api-docs/testnet/fix-api#types "Direct link to Types")
...
## Message Components[​](/docs/binance-spot-api-docs/testnet/fix-api#message-components "Direct link to Message Components")
...
### Header[​](/docs/binance-spot-api-docs/testnet/fix-api#header "Direct link to Header")
...
`0` \- [HEARTBEAT](/docs/binance-spot-api-docs/testnet/fix-api#heartbeat)   
  
`1` \- [TEST_REQUEST](/docs/binance-spot-api-docs/testnet/fix-api#testrequest)   
  
`3` \- [REJECT](/docs/binance-spot-api-docs/testnet/fix-api#reject)   
  
`5` \- [LOGOUT](/docs/binance-spot-api-docs/testnet/fix-api#logout)   
  
`8` \- [EXECUTION_REPORT](/docs/binance-spot-api-docs/testnet/fix-api#executionreport)   
  
`9` \- [ORDER_CANCEL_REJECT](/docs/binance-spot-api-docs/testnet/fix-api#ordercancelreject)   
  
`A` \- [LOGON](/docs/binance-spot-api-docs/testnet/fix-api#logon-main)   
  
`D` \- [NEW_ORDER_SINGLE](/docs/binance-spot-api-docs/testnet/fix-api#newordersingle)   
  
`E` \- [NEW_ORDER_LIST](/docs/binance-spot-api-docs/testnet/fix-api#neworderlist)   
  
`F` \- [ORDER_CANCEL_REQUEST](/docs/binance-spot-api-docs/testnet/fix-api#ordercancelrequest)   
  
`N` \- [LIST_STATUS](/docs/binance-spot-api-docs/testnet/fix-api#liststatus)   
  
`q` \- [ORDER_MASS_CANCEL_REQUEST](/docs/binance-spot-api-docs/testnet/fix-api#ordermasscancelrequest)   
  
`r` \- [ORDER_MASS_CANCEL_REPORT](/docs/binance-spot-api-docs/testnet/fix-api#ordermasscancelreport)   
  
`XCN` \- [ORDER_CANCEL_REQUEST_AND_NEW_ORDER_SINGLE](/docs/binance-spot-api-docs/testnet/fix-api#ordercancelrequestandnewordersingle)   
  
`XLQ` \- [LIMIT_QUERY](/docs/binance-spot-api-docs/testnet/fix-api#limitquery)   
  
`XLR` \- [LIMIT_RESPONSE](/docs/binance-spot-api-docs/testnet/fix-api#limitresponse)   
  
`B` \- [NEWS](/docs/binance-spot-api-docs/testnet/fix-api#news)   
  
`x`\- [INSTRUMENT_LIST_REQUEST](/docs/binance-spot-api-docs/testnet/fix-api#instrumentlistrequest)   
  
`y` \- [INSTRUMENT_LIST](/docs/binance-spot-api-docs/testnet/fix-api#instrumentlist)   
  
`V` \- [MARKET_DATA_REQUEST](/docs/binance-spot-api-docs/testnet/fix-api#marketdatarequest)   
  
`Y` \- [MARKET_DATA_REQUEST_REJECT](/docs/binance-spot-api-docs/testnet/fix-api#marketdatarequestreject)   
  
`W` \- [MARKET_DATA_SNAPSHOT](/docs/binance-spot-api-docs/testnet/fix-api#marketdatasnapshot)   
  
`X` \- [MARKET_DATA_INCREMENTAL_REFRESH](/docs/binance-spot-api-docs/testnet/fix-api#marketdataincrementalrefresh)   
  
`XAK` \- [ORDER_AMEND_KEEP_PRIORITY_REQUEST](/docs/binance-spot-api-docs/testnet/fix-api#orderamendkeeppriorityrequest)   
  
`XAR` \- [ORDER_AMEND_REJECT](/docs/binance-spot-api-docs/testnet/fix-api#orderamendreject)  
...
Defaults to `5000` milliseconds in [Logon`<A>`](/docs/binance-spot-api-docs/testnet/fix-api#logon-request) and has a max value of `60000` milliseconds.   
Supports up to three decimal places of precision (e.g., 6000.346) so that microseconds may be specified.  
  
### Trailer[​](/docs/binance-spot-api-docs/testnet/fix-api#trailer "Direct link to Trailer")
...
## Administrative Messages[​](/docs/binance-spot-api-docs/testnet/fix-api#administrative-messages "Direct link to Administrative Messages")

### Heartbeat `<0>`[​](/docs/binance-spot-api-docs/testnet/fix-api#heartbeat-0 "Direct link to heartbeat-0")

Sent by the server if there is no outgoing traffic during the heartbeat interval (`HeartBtInt (108)` in [Logon`<A>`](/docs/binance-spot-api-docs/testnet/fix-api#logon-main)).

Sent by the client to indicate that the session is healthy.

Sent by the client or the server in response to a [TestRequest`<1>`](/docs/binance-spot-api-docs/testnet/fix-api#testrequest) message.

Tag| Name| Type| Required| Description  
---|---|---|---|---  
112| TestReqID| STRING| N| When Heartbeat`<35>` is sent in response to TestRequest`<1>`, must mirror the value in TestRequest`<1>`.  
  
### TestRequest `<1>`[​](/docs/binance-spot-api-docs/testnet/fix-api#testrequest-1 "Direct link to testrequest-1")

Sent by the server if there is no incoming traffic during the heartbeat interval (`HeartBtInt (108)` in [Logon`<A>`](/docs/binance-spot-api-docs/testnet/fix-api#logon-main)).

Sent by the client to request a [Heartbeat`<0>`](/docs/binance-spot-api-docs/testnet/fix-api#heartbeat) response.
...
### Reject `<3>`[​](/docs/binance-spot-api-docs/testnet/fix-api#reject-3 "Direct link to reject-3")

Sent by the server in response to an invalid message that cannot be processed.

Sent by the server if a new connection cannot be accepted. Please refer to [Connection Limits](/docs/binance-spot-api-docs/testnet/fix-api#connection-limits).
...
`99` \- OTHER  
25016| ErrorCode| INT| N| API error code (see [Error Codes](/docs/binance-spot-api-docs/testnet/errors)).  
58| Text| STRING| N| Human-readable error message.  
  
### Logon `<A>`[​](/docs/binance-spot-api-docs/testnet/fix-api#logon-a "Direct link to logon-a")
...
#### Logon Request[​](/docs/binance-spot-api-docs/testnet/fix-api#logon-request "Direct link to Logon Request")
...
96| RawData| DATA| Y| Signature. [How to sign Logon`<A>` request](/docs/binance-spot-api-docs/testnet/fix-api#signaturecomputation).  
...
Please refer to [On message order processing](/docs/binance-spot-api-docs/testnet/fix-api#orderedmode) for more information.  
25036| ResponseMode| INT| N| Please refer to [Response Mode](/docs/binance-spot-api-docs/testnet/fix-api#responsemode).  
...
#### Logon Response[​](/docs/binance-spot-api-docs/testnet/fix-api#logon-response "Direct link to Logon Response")
...
### Logout `<5>`[​](/docs/binance-spot-api-docs/testnet/fix-api#logout-5 "Direct link to logout-5")
...
### News `<B>`[​](/docs/binance-spot-api-docs/testnet/fix-api#news-b "Direct link to news-b")

When the server enters maintenance, a `News` message will be sent to clients **every 10 seconds** , counting down to disconnection. When the countdown ends, clients are logged out and their sessions are closed.
...
### Resend Request `<2>`[​](/docs/binance-spot-api-docs/testnet/fix-api#resend-request-2 "Direct link to resend-request-2")

Resend requests are currently not supported.

## Application Messages[​](/docs/binance-spot-api-docs/testnet/fix-api#application-messages "Direct link to Application Messages")

### Order Entry Messages[​](/docs/binance-spot-api-docs/testnet/fix-api#order-entry-messages "Direct link to Order Entry Messages")

> [!NOTE] The messages below can only be used for the FIX Order Entry and FIX Drop Copy Sessions.

#### NewOrderSingle `<D>`[​](/docs/binance-spot-api-docs/testnet/fix-api#newordersingle-d "Direct link to newordersingle-d")
...
Please refer to [Supported Order Types](/docs/binance-spot-api-docs/testnet/fix-api#ordertype) for supported field combinations.

> [!NOTE] Many fields become required based on the order type. Please refer to [Supported Order Types](/docs/binance-spot-api-docs/testnet/fix-api#NewOrderSingle-required-fields).

Tag| Name| Type| Required| Description  
---|---|---|---|---  
11| ClOrdID| STRING| Y| `ClOrdID` to be assigned to the order.  
38| OrderQty| QTY| N| Quantity of the order  
40| OrdType| CHAR| Y| See the [table](/docs/binance-spot-api-docs/testnet/fix-api#ordertype) to understand supported order types and the required fields to use them.  
...
`1` \- ACTIVATE  
1102| TriggerPrice| PRICE| N| Activation price for contingent orders. See [table](/docs/binance-spot-api-docs/testnet/fix-api#ordertype)  
1107| TriggerPriceType| CHAR| N| Possible values:   
  
`2` \- LAST_TRADE  
1109| TriggerPriceDirection| CHAR| N| Used to differentiate between StopLoss and TakeProfit orders. See [table](/docs/binance-spot-api-docs/testnet/fix-api#ordertype).  
...
**Response:**
  ...
  * [ExecutionReport`<8>`](/docs/binance-spot-api-docs/testnet/fix-api#executionreport) with `ExecType (150)` value `NEW (0)` if the order was accepted.
  * [ExecutionReport`<8>`](/docs/binance-spot-api-docs/testnet/fix-api#executionreport) with `ExecType (150)` value `REJECTED (8)` if the order was rejected.
  * [Reject`<3>`](/docs/binance-spot-api-docs/testnet/fix-api#reject) if the message is rejected.



##### Supported Order Types[​](/docs/binance-spot-api-docs/testnet/fix-api#supported-order-types "Direct link to Supported Order Types")
...
#### ExecutionReport `<8>`[​](/docs/binance-spot-api-docs/testnet/fix-api#executionreport-8 "Direct link to executionreport-8")

Sent by the server whenever an order state changes.

> [!NOTE]
> 
>   * By default, ExecutionReport`<8>` is sent for all orders of an account, including those submitted in different connections. Please see [Response Mode](/docs/binance-spot-api-docs/testnet/fix-api#responsemode) for other behavior options.
...
#### OrderCancelRequest `<F>`[​](/docs/binance-spot-api-docs/testnet/fix-api#ordercancelrequest-f "Direct link to ordercancelrequest-f")
...
**Response:**
  ...
  * [ExecutionReport`<8>`](/docs/binance-spot-api-docs/testnet/fix-api#executionreport) with `ExecType (150)` value `CANCELED (4)` for each canceled order.
  * [ListStatus`<N>`](/docs/binance-spot-api-docs/testnet/fix-api#liststatus) if orders in an order list were canceled.
  * [OrderCancelReject`<9>`](/docs/binance-spot-api-docs/testnet/fix-api#ordercancelreject) if cancellation was rejected.
  * [Reject`<3>`](/docs/binance-spot-api-docs/testnet/fix-api#reject) if the message is rejected.



#### OrderCancelReject `<9>`[​](/docs/binance-spot-api-docs/testnet/fix-api#ordercancelreject-9 "Direct link to ordercancelreject-9")

Sent by the server when [OrderCancelRequest`<F>`](/docs/binance-spot-api-docs/testnet/fix-api#ordercancelrequest) has failed.
...
25016| ErrorCode| INT| Y| API error code (see [Error Codes](/docs/binance-spot-api-docs/testnet/errors)).  
...
#### OrderCancelRequestAndNewOrderSingle `<XCN>`[​](/docs/binance-spot-api-docs/testnet/fix-api#ordercancelrequestandnewordersingle-xcn "Direct link to ordercancelrequestandnewordersingle-xcn")
...
Please refer to [Supported Order Types](/docs/binance-spot-api-docs/testnet/fix-api#ordertype) for supported field combinations when describing the new order.
...
**Response:**
  ...
  * [ExecutionReport`<8>`](/docs/binance-spot-api-docs/testnet/fix-api#executionreport) with `ExecType (150)` value `CANCELED (4)` for the canceled order.
  * [ExecutionReport`<8>`](/docs/binance-spot-api-docs/testnet/fix-api#executionreport) with `ExecType (150)` value `NEW (0)` for the new order.
  * [ExecutionReport`<8>`](/docs/binance-spot-api-docs/testnet/fix-api#executionreport) with `ExecType (150)` value `REJECTED (8)` if the new order was rejected.
  * [OrderCancelReject`<9>`](/docs/binance-spot-api-docs/testnet/fix-api#ordercancelreject) if the cancellation was rejected.
  * [Reject`<3>`](/docs/binance-spot-api-docs/testnet/fix-api#reject) if the message is rejected.



#### OrderMassCancelRequest `<q>`[​](/docs/binance-spot-api-docs/testnet/fix-api#ordermasscancelrequest-q "Direct link to ordermasscancelrequest-q")
...
**Responses:**
  ...
  * [ExecutionReport`<8>`](/docs/binance-spot-api-docs/testnet/fix-api#executionreport) with `ExecType (150)` value `CANCELED (4)` for the every order canceled.
  * [OrderMassCancelReport`<r>`](/docs/binance-spot-api-docs/testnet/fix-api#ordermasscancelreport) with `MassCancelResponse (531)` field indicating whether the message is accepted or rejected.
  * [Reject`<3>`](/docs/binance-spot-api-docs/testnet/fix-api#reject) if the message is rejected.



#### OrderMassCancelReport `<r>`[​](/docs/binance-spot-api-docs/testnet/fix-api#ordermasscancelreport-r "Direct link to ordermasscancelreport-r")

Sent by the server in response to [OrderMassCancelRequest`<q>`](/docs/binance-spot-api-docs/testnet/fix-api#ordermasscancelrequest).
...
#### NewOrderList `<E>`[​](/docs/binance-spot-api-docs/testnet/fix-api#neworderlist-e "Direct link to neworderlist-e")
...
Orders in an order list are contingent on one another. Please refer to [Supported Order List Types](/docs/binance-spot-api-docs/testnet/fix-api#order-list-types) for supported order types and triggering instructions.
...
#### Supported Order List Types[​](/docs/binance-spot-api-docs/testnet/fix-api#supported-order-list-types "Direct link to Supported Order List Types")
...
#### ListStatus `<N>`[​](/docs/binance-spot-api-docs/testnet/fix-api#liststatus-n "Direct link to liststatus-n")

Sent by the server whenever an order list state changes.

> [!NOTE] By default, ListStatus`<N>` is sent for all order lists of an account, including those submitted in different connections. Please see [Response Mode](/docs/binance-spot-api-docs/testnet/fix-api#responsemode) for other behavior options.
...
#### OrderAmendKeepPriorityRequest `<XAK>`[​](/docs/binance-spot-api-docs/testnet/fix-api#orderamendkeeppriorityrequest-xak "Direct link to orderamendkeeppriorityrequest-xak")
...
**Response:**
  ...
  * [Reject `<3>`](/docs/binance-spot-api-docs/testnet/fix-api#reject) if the incoming request is invalid either due to missing required fields, invalid fields, refers to an invalid symbol, or exceeds the message limit.
  * [OrderAmendReject `<XAR>`](/docs/binance-spot-api-docs/testnet/fix-api#orderamendreject) if failed due to insufficient order rate limits, pointing to a non-existent order, quantity is invalid, etc.
  * [ExecutionReport `<8>`](/docs/binance-spot-api-docs/testnet/fix-api#executionreport) if the request succeeded for amending a single order.
  * [ExecutionReport `<8>`](/docs/binance-spot-api-docs/testnet/fix-api#executionreport) \+ [ListStatus `<N>`](/docs/binance-spot-api-docs/testnet/fix-api#liststatus) if the request succeeded for amending an order which is part of an Order list.



### OrderAmendReject `<XAR>`[​](/docs/binance-spot-api-docs/testnet/fix-api#orderamendreject-xar "Direct link to orderamendreject-xar")
...
### Limit Messages[​](/docs/binance-spot-api-docs/testnet/fix-api#limit-messages "Direct link to Limit Messages")

#### LimitQuery `<XLQ>`[​](/docs/binance-spot-api-docs/testnet/fix-api#limitquery-xlq "Direct link to limitquery-xlq")
...
#### LimitResponse `<XLR>`[​](/docs/binance-spot-api-docs/testnet/fix-api#limitresponse-xlr "Direct link to limitresponse-xlr")

Sent by the server in response to [LimitQuery`<XLQ>`](/docs/binance-spot-api-docs/testnet/fix-api#limitquery).
...
### Market Data Messages[​](/docs/binance-spot-api-docs/testnet/fix-api#market-data-messages "Direct link to Market Data Messages")

> [!NOTE] The messages below can only be used for the FIX Market Data.

#### InstrumentListRequest `<x>`[​](/docs/binance-spot-api-docs/testnet/fix-api#instrumentlistrequest-x "Direct link to instrumentlistrequest-x")
...
#### InstrumentList `<y>`[​](/docs/binance-spot-api-docs/testnet/fix-api#instrumentlist-y "Direct link to instrumentlist-y")

Sent by the server in a response to the [InstrumentListRequest`<x>`](/docs/binance-spot-api-docs/testnet/fix-api#instrumentlistrequest).

> [!NOTE] More detailed symbol information is available through the [exchangeInfo](https://github.com/binance/binance-spot-api-docs/blob/master/testnet/rest-api/general-endpoints.md#exchange-information) endpoint.
...
=>562| MinTradeVol| QTY| N| Corresponds to the [LOT_SIZE](/docs/binance-spot-api-docs/testnet/filters#lot_size) filter  
=>1140| MaxTradeVol| QTY| N| Corresponds to the [LOT_SIZE](/docs/binance-spot-api-docs/testnet/filters#lot_size) filter  
=>25039| MinQtyIncrement| QTY| N| Corresponds to the [LOT_SIZE](/docs/binance-spot-api-docs/testnet/filters#lot_size) filter  
=>25040| MarketMinTradeVol| QTY| N| Corresponds to the [MARKET_LOT_SIZE](/docs/binance-spot-api-docs/testnet/filters#market_lot_size) filter  
=>25041| MarketMaxTradeVol| QTY| N| Corresponds to the [MARKET_LOT_SIZE](/docs/binance-spot-api-docs/testnet/filters#market_lot_size) filter  
=>25042| MarketMinQtyIncrement| QTY| N| Corresponds to the [MARKET_LOT_SIZE](/docs/binance-spot-api-docs/testnet/filters#market_lot_size) filter  
=>969| MinPriceIncrement| PRICE| N| Corresponds to the [PRICE](/docs/binance-spot-api-docs/testnet/filters#price_filter) filter  
=>2551| StartPriceRange| PRICE| N| Corresponds to the [PRICE](/docs/binance-spot-api-docs/testnet/filters#price_filter) filter  
=>2552| EndPriceRange| PRICE| N| Corresponds to the [PRICE](/docs/binance-spot-api-docs/testnet/filters#price_filter) filter  
...
#### MarketDataRequest `<V>`[​](/docs/binance-spot-api-docs/testnet/fix-api#marketdatarequest-v "Direct link to marketdatarequest-v")
...
> [!NOTE] In the [Individual Symbol Book Ticker Stream](/docs/binance-spot-api-docs/testnet/fix-api#symbolbooktickerstream), when `MDUpdateAction` is set to `CHANGE(1)` in a [MarketDataIncrementalRefresh`<X>`](/docs/binance-spot-api-docs/testnet/fix-api#marketdataincrementalrefresh) message sent from the server, it replaces the previous best quote.
...
**Fields required to subscribe:**
  ...
  * `MDEntryType` with value `BID(0)`
  * `MDEntryType` with value `OFFER(1)`
  * `MarketDepth` with a value between `2` and `5000`, which controls the size of the initial snapshot and has no effect on subsequent [MarketDataIncrementalRefresh`<X>`](/docs/binance-spot-api-docs/testnet/fix-api#marketdataincrementalrefresh) messages



**Update Speed:** 100ms

> [!NOTE] Since the [MarketDataSnapshot`<W>`](/docs/binance-spot-api-docs/testnet/fix-api#marketdatasnapshot) have a limit on the number of price levels (5000 on each side maximum), you won't learn the quantities for the levels outside of the initial snapshot unless they change. So be careful when using the information for those levels, since they might not reflect the full view of the order book. However, for most use cases, seeing 5000 levels on each side is enough to understand the market and trade effectively.
...
### MarketDataRequestReject `<Y>`[​](/docs/binance-spot-api-docs/testnet/fix-api#marketdatarequestreject-y "Direct link to marketdatarequestreject-y")

Sent by the server in a response to an invalid MarketDataRequest `<V>`.

Tag| Name| Type| Required| Description  
---|---|---|---|---  
262| MDReqID| STRING| Y| ID of the invalid [MarketDataRequest`<V>`](/docs/binance-spot-api-docs/testnet/fix-api#marketdatarequest)  
281| MDReqRejReason| CHAR| N| Possible values:   
  
`1` \- DUPLICATE_MDREQID   
  
`2` \- TOO_MANY_SUBSCRIPTIONS  
25016| ErrorCode| INT| N| API Error code. See [Errors](/docs/binance-spot-api-docs/testnet/errors)  
58| Text| STRING| N| Human-readable error message.  
...
### MarketDataSnapshot `<W>`[​](/docs/binance-spot-api-docs/testnet/fix-api#marketdatasnapshot-w "Direct link to marketdatasnapshot-w")

Sent by the server in response to a [MarketDataRequest`<V>`](/docs/binance-spot-api-docs/testnet/fix-api#marketdatarequest), activating [Individual Symbol Book Ticker Stream](/docs/binance-spot-api-docs/testnet/fix-api#symbolbooktickerstream) or [Diff. Depth Stream](/docs/binance-spot-api-docs/testnet/fix-api#diffdepthstream) subscriptions.
...
### MarketDataIncrementalRefresh `<X>`[​](/docs/binance-spot-api-docs/testnet/fix-api#marketdataincrementalrefresh-x "Direct link to marketdataincrementalrefresh-x")
...
`2` \- SELL  
=>25043| FirstBookUpdateID| INT| N| Only present in [Diff. Depth Stream](/docs/binance-spot-api-docs/testnet/fix-api#diffdepthstream).   
...
=>25044| LastBookUpdateID| INT| N| Only present in [Diff. Depth Stream](/docs/binance-spot-api-docs/testnet/fix-api#diffdepthstream) and [Individual Symbol Book Ticker Stream](/docs/binance-spot-api-docs/testnet/fix-api#symbolbooktickerstream).   
...
### SBE[​](/docs/binance-spot-api-docs/testnet/fix-api#sbe "Direct link to SBE")
...
### Endpoints[​](/docs/binance-spot-api-docs/testnet/fix-api#endpoints "Direct link to Endpoints")
...
### FIX SBE encoding layout[​](/docs/binance-spot-api-docs/testnet/fix-api#fix-sbe-encoding-layout "Direct link to FIX SBE encoding layout")
...
### Logon[​](/docs/binance-spot-api-docs/testnet/fix-api#logon "Direct link to Logon")

The logon signature (RawData) is computed as documented in the [signature computation](/docs/binance-spot-api-docs/testnet/fix-api#signaturecomputation) section.

#### Sample FIX SBE `Logon` request message[​](/docs/binance-spot-api-docs/testnet/fix-api#sample-fix-sbe-logon-request-message "Direct link to sample-fix-sbe-logon-request-message")
...
### FIX vs. FIX SBE[​](/docs/binance-spot-api-docs/testnet/fix-api#fix-vs-fix-sbe "Direct link to FIX vs. FIX SBE")
...
### Limits[​](/docs/binance-spot-api-docs/testnet/fix-api#limits-1 "Direct link to Limits")

Connection limits are shared between FIX and FIX SBE.

### Errors[​](/docs/binance-spot-api-docs/testnet/fix-api#errors "Direct link to Errors")
...
### FAQ[​](/docs/binance-spot-api-docs/testnet/fix-api#faq "Direct link to FAQ")
...
## General API Information[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#general-api-information "General API Information的直接链接")

  * FIX connections require TLS encryption. Please either use native TCP+TLS connection or set up a local proxy such as [stunnel](https://www.stunnel.org/) to handle TLS encryption.
  * APIs have a timeout of 10 seconds when processing a request. If a response from the Matching Engine takes longer than this, the API responds with "Timeout waiting for response from backend server. Send status unknown; execution status unknown." [(-1007 TIMEOUT)](/docs/zh-CN/binance-spot-api-docs/testnet/errors#-1007-timeout)
    * This does not always mean that the request failed in the Matching Engine.
    * If the status of the request has not appeared in [User Data Stream](/docs/zh-CN/binance-spot-api-docs/testnet/user-data-stream), please perform an API query for its status.
  * If your request contains a symbol name containing non-ASCII characters, then the response may contain non-ASCII characters encoded in UTF-8.
  * To ensure uninterrupted connectivity, please make sure that your client sends **SNI (Server Name Indication)** during the TLS handshake and performs certificate validation against the intended hostname.   
Clients that do not send SNI may receive an unexpected certificate, which can result in TLS handshake or hostname verification failures.
...
### NodeJS[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#nodejs "NodeJS的直接链接")
...
### Other languages/custom TLS implementations[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#other-languagescustom-tls-implementations "Other languages/custom TLS implementations的直接链接")
...
### FIX API Order Entry sessions[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#fix-api-order-entry-sessions "FIX API Order Entry sessions的直接链接")

  * Endpoint is: `tcp+tls://fix-oe.testnet.binance.vision:9000`
  * Supports placing orders, canceling orders, and querying current limit usage.
  * Supports receiving all of the account's [ExecutionReport`<8>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#executionreport) and [List Status`<N>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#liststatus).
  * Only API keys with `FIX_API` are allowed to connect.
  * QuickFIX Schema can be found [here](https://github.com/binance/binance-spot-api-docs/blob/master/fix/schemas/spot-fix-oe.xml).



### FIX API Drop Copy sessions[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#fix-api-drop-copy-sessions "FIX API Drop Copy sessions的直接链接")
...
### FIX API Market Data sessions[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#fix-api-market-data-sessions "FIX API Market Data sessions的直接链接")
...
### FIX Connection Lifecycle[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#fix-connection-lifecycle "FIX Connection Lifecycle的直接链接")

  * All FIX API sessions will remain open for as long as possible, on a best-effort basis.
  * There is no minimum connection time guarantee; a server can enter maintenance at any time. 
    * When a server enters maintenance, a [News `<B>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#news) message will be sent to clients **every 10 seconds until disconnection** , prompting clients to reconnect. Upon receiving this message, a client is expected to establish a new session and close the old one. If the client does not close the old session before the server disconnects it, the server will proceed to log it out and close the session.
  * After connecting, the client must send a Logon `<A>` request. For more information please refer to [How to sign a Logon request](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#signaturecomputation).
  * The client should send a Logout `<5>` message to close the session before disconnecting. Failure to send the logout message will result in the session’s `SenderCompID (49)` being unusable for new session establishment for a duration of 2x the `HeartInt (108)` interval.
  * The system allows negotiation of the `HeartInt (108)` value during the logon process. Accepted values range between 5 and 60 seconds. 
    * If the server has not sent any messages within a `HeartInt (108)` interval, a [HeartBeat `<0>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#heartbeat) will be sent.
    * If the server has not received any messages within a `HeartInt (108)` interval, a [TestRequest `<1>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#testrequest) will be sent. If the server does not receive a HeartBeat `<0>` containing the expected `TestReqID (112)` from the client within `HeartInt (108)` seconds, the server will send a Logout `<5>` message and close the connection.
    * If the client has not received any messages within a `HeartInt (108)` interval, the client is responsible for sending a TestRequest `<1>` to ensure the connection is healthy. Upon receiving such a TestRequest `<1>`, the server will respond with a Heartbeat `<0>` containing the expected `TestReqID (112)`. If the client does not receive the server’s response within a `HeartInt (108)` interval, the client should close the session and connection and establish new ones.



### API Key Permissions[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#api-key-permissions "API Key Permissions的直接链接")
...
### On message processing order[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#on-message-processing-order "On message processing order的直接链接")

The `MessageHandling (25035)` field required in the initial [Logon`<A>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#logon-request) message controls whether messages from the client may be reordered before they are processed by the Matching Engine.
...
### Response Mode[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#response-mode "Response Mode的直接链接")

By default, all concurrent order entry sessions receive all of the account's successful [ExecutionReport`<8>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#executionreport) and [ListStatus`<N>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#liststatus) messages, including those in response to orders placed from other FIX sessions and via non-FIX APIs.

Use the `ResponseMode (25036)` field in the initial [Logon`<A>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#logon-request) message to change this behavior.
...
### Timing Security[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#timing-security "Timing Security的直接链接")
...
### How to sign Logon `<A>` request[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#how-to-sign-logon-a-request "how-to-sign-logon-a-request的直接链接")

The [Logon`<A>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#logon-main) message authenticates your connection to the FIX API. This must be the first message sent by the client.
...
## Limits[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#limits "Limits的直接链接")

### Message Limits[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#message-limits "Message Limits的直接链接")

  * Each connection has a limit on **how many messages can be sent to the exchange**.
  * The message limit **does not count the messages sent in response to the client**.
  * Breaching the message limit results in immediate [Logout `<5>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#logout) and disconnection.
  * To understand current limits and usage, please send a [LimitQuery`<XLQ>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#limitquery) message. A [LimitResponse`<XLR>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#limitresponse) message will be sent in response, containing information about Order Rate Limits and Message Limits.
  * FIX Order entry sessions have a limit of 10,000 messages every 10 seconds.
  * FIX Drop Copy sessions have a limit of 60 messages every 60 seconds.
  * FIX Market Data sessions have a limit of 2000 messages every 60 seconds.
...
### Connection Limits[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#connection-limits "Connection Limits的直接链接")

  * Each Account has a limit on how many TCP connections can be established at the same time.
  * The limit is reduced when the TCP connection is closed. If the reduction of connections is not immediate, please wait up to twice the value of `HeartBtInt (108)` for the change to take effect. For example, if the current value of `HeartBtInt` is 5, please wait up to 10 seconds.
  * Upon breaching the limit a [Reject `<3>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#reject) will be sent containing information about the connection limit breach and the current limit.
  * FIX Order Entry limits: 
    * 15 connection attempts within 30 seconds
    * Maximum of 10 concurrent TCP connections per account
  * FIX Drop Copy limits: 
...
### Unfilled Order Count[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#unfilled-order-count "Unfilled Order Count的直接链接")

  * To understand how many orders you have placed within a certain time interval, please send a [LimitQuery`<XLQ>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#limitquery) message. A [LimitResponse`<XLR>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#limitresponse) message will be sent in response, containing information about Unfilled Order Count and Message Limits.
  * **Please note that if your orders are consistently filled by trades, you can continuously place orders on the API**. For more information, please see [Spot Unfilled Order Count Rules](/docs/zh-CN/binance-spot-api-docs/faqs/order_count_decrement.md).
  * If you exceed the unfilled order count your message will be rejected, and information will be transferred back to you in a reject message specific to that endpoint.
  * **The number of unfilled orders is tracked for each account.**



## Error Handling[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#error-handling "Error Handling的直接链接")

Client messages that contain syntax errors, missing required fields, or refer to unknown symbols will be rejected by the server with a [Reject `<3>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#reject) message.

If a valid message cannot be processed and is rejected, an appropriate reject response will be sent. Please refer to the individual message documentation for possible responses.

Please refer to the `Text (58)` and `ErrorCode (25016)` fields in responses for the reject reason.

The list of error codes can be found on the [Error codes](/docs/zh-CN/binance-spot-api-docs/testnet/errors) page.

## Types[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#types "Types的直接链接")
...
## Message Components[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#message-components "Message Components的直接链接")
...
### Header[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#header "Header的直接链接")
...
`0` \- [HEARTBEAT](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#heartbeat)   
  
`1` \- [TEST_REQUEST](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#testrequest)   
  
`3` \- [REJECT](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#reject)   
  
`5` \- [LOGOUT](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#logout)   
  
`8` \- [EXECUTION_REPORT](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#executionreport)   
  
`9` \- [ORDER_CANCEL_REJECT](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordercancelreject)   
  
`A` \- [LOGON](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#logon-main)   
  
`D` \- [NEW_ORDER_SINGLE](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#newordersingle)   
  
`E` \- [NEW_ORDER_LIST](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#neworderlist)   
  
`F` \- [ORDER_CANCEL_REQUEST](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordercancelrequest)   
  
`N` \- [LIST_STATUS](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#liststatus)   
  
`q` \- [ORDER_MASS_CANCEL_REQUEST](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordermasscancelrequest)   
  
`r` \- [ORDER_MASS_CANCEL_REPORT](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordermasscancelreport)   
  
`XCN` \- [ORDER_CANCEL_REQUEST_AND_NEW_ORDER_SINGLE](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordercancelrequestandnewordersingle)   
  
`XLQ` \- [LIMIT_QUERY](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#limitquery)   
  
`XLR` \- [LIMIT_RESPONSE](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#limitresponse)   
  
`B` \- [NEWS](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#news)   
  
`x`\- [INSTRUMENT_LIST_REQUEST](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#instrumentlistrequest)   
  
`y` \- [INSTRUMENT_LIST](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#instrumentlist)   
  
`V` \- [MARKET_DATA_REQUEST](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#marketdatarequest)   
  
`Y` \- [MARKET_DATA_REQUEST_REJECT](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#marketdatarequestreject)   
  
`W` \- [MARKET_DATA_SNAPSHOT](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#marketdatasnapshot)   
  
`X` \- [MARKET_DATA_INCREMENTAL_REFRESH](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#marketdataincrementalrefresh)   
  
`XAK` \- [ORDER_AMEND_KEEP_PRIORITY_REQUEST](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#orderamendkeeppriorityrequest)   
  
`XAR` \- [ORDER_AMEND_REJECT](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#orderamendreject)  
49| SenderCompID| STRING| Y| Must be unique across an account's active sessions.   
...
Defaults to `5000` milliseconds in [Logon`<A>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#logon-request) and has a max value of `60000` milliseconds.   
Supports up to three decimal places of precision (e.g., 6000.346) so that microseconds may be specified.  
  
### Trailer[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#trailer "Trailer的直接链接")
...
## Administrative Messages[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#administrative-messages "Administrative Messages的直接链接")

### Heartbeat `<0>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#heartbeat-0 "heartbeat-0的直接链接")

Sent by the server if there is no outgoing traffic during the heartbeat interval (`HeartBtInt (108)` in [Logon`<A>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#logon-main)).

Sent by the client to indicate that the session is healthy.

Sent by the client or the server in response to a [TestRequest`<1>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#testrequest) message.

Tag| Name| Type| Required| Description  
---|---|---|---|---  
112| TestReqID| STRING| N| When Heartbeat`<35>` is sent in response to TestRequest`<1>`, must mirror the value in TestRequest`<1>`.  
  
### TestRequest `<1>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#testrequest-1 "testrequest-1的直接链接")

Sent by the server if there is no incoming traffic during the heartbeat interval (`HeartBtInt (108)` in [Logon`<A>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#logon-main)).

Sent by the client to request a [Heartbeat`<0>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#heartbeat) response.
...
### Reject `<3>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#reject-3 "reject-3的直接链接")

Sent by the server in response to an invalid message that cannot be processed.

Sent by the server if a new connection cannot be accepted. Please refer to [Connection Limits](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#connection-limits).
...
`99` \- OTHER  
25016| ErrorCode| INT| N| API error code (see [Error Codes](/docs/zh-CN/binance-spot-api-docs/testnet/errors)).  
58| Text| STRING| N| Human-readable error message.  
  
### Logon `<A>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#logon-a "logon-a的直接链接")
...
#### Logon Request[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#logon-request "Logon Request的直接链接")
...
108| HeartBtInt| INT| Y| Required to be within range [5, 60]. Heartbeat interval in seconds.  
95| RawDataLength| LENGTH| Y| Length of the `RawData (96)` field that comes strictly after this field.  
96| RawData| DATA| Y| Signature. [How to sign Logon`<A>` request](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#signaturecomputation).  
141| ResetSeqNumFlag| BOOLEAN| Y| Required to be `Y`.  
553| Username| STRING| Y| API key. **Only Ed25519 API keys are supported.**  
25035| MessageHandling| INT| Y| Possible values:   
...
Please refer to [On message order processing](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#orderedmode) for more information.  
25036| ResponseMode| INT| N| Please refer to [Response Mode](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#responsemode).  
9406| DropCopyFlag| BOOLEAN| N| Must be set to 'Y' when logging into Drop Copy sessions.  
...
#### Logon Response[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#logon-response "Logon Response的直接链接")
...
### Logout `<5>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#logout-5 "logout-5的直接链接")
...
### News `<B>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#news-b "news-b的直接链接")
...
### Resend Request `<2>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#resend-request-2 "resend-request-2的直接链接")

Resend requests are currently not supported.

## Application Messages[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#application-messages "Application Messages的直接链接")

### Order Entry Messages[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#order-entry-messages "Order Entry Messages的直接链接")

> [!NOTE] The messages below can only be used for the FIX Order Entry and FIX Drop Copy Sessions.

#### NewOrderSingle `<D>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#newordersingle-d "newordersingle-d的直接链接")
...
Please refer to [Supported Order Types](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordertype) for supported field combinations.

> [!NOTE] Many fields become required based on the order type. Please refer to [Supported Order Types](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#NewOrderSingle-required-fields).

Tag| Name| Type| Required| Description  
---|---|---|---|---  
11| ClOrdID| STRING| Y| `ClOrdID` to be assigned to the order.  
38| OrderQty| QTY| N| Quantity of the order  
40| OrdType| CHAR| Y| See the [table](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordertype) to understand supported order types and the required fields to use them.  
...
`1` \- ACTIVATE  
1102| TriggerPrice| PRICE| N| Activation price for contingent orders. See [table](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordertype)  
1107| TriggerPriceType| CHAR| N| Possible values:   
  
`2` \- LAST_TRADE  
1109| TriggerPriceDirection| CHAR| N| Used to differentiate between StopLoss and TakeProfit orders. See [table](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordertype).  
...
**Response:**
  ...
  * [ExecutionReport`<8>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#executionreport) with `ExecType (150)` value `NEW (0)` if the order was accepted.
  * [ExecutionReport`<8>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#executionreport) with `ExecType (150)` value `REJECTED (8)` if the order was rejected.
  * [Reject`<3>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#reject) if the message is rejected.



##### Supported Order Types[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#supported-order-types "Supported Order Types的直接链接")
...
#### ExecutionReport `<8>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#executionreport-8 "executionreport-8的直接链接")

Sent by the server whenever an order state changes.

> [!NOTE]
> 
>   * By default, ExecutionReport`<8>` is sent for all orders of an account, including those submitted in different connections. Please see [Response Mode](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#responsemode) for other behavior options.
>   * FIX API should give better performance for ExecutionReport`<8>` push.
> 
...
#### OrderCancelRequest `<F>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordercancelrequest-f "ordercancelrequest-f的直接链接")
...
**Response:**
  ...
  * [ExecutionReport`<8>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#executionreport) with `ExecType (150)` value `CANCELED (4)` for each canceled order.
  * [ListStatus`<N>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#liststatus) if orders in an order list were canceled.
  * [OrderCancelReject`<9>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordercancelreject) if cancellation was rejected.
  * [Reject`<3>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#reject) if the message is rejected.



#### OrderCancelReject `<9>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordercancelreject-9 "ordercancelreject-9的直接链接")

Sent by the server when [OrderCancelRequest`<F>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordercancelrequest) has failed.
...
`1` \- ORDER_CANCEL_REQUEST  
25016| ErrorCode| INT| Y| API error code (see [Error Codes](/docs/zh-CN/binance-spot-api-docs/testnet/errors)).  
58| Text| STRING| Y| Human-readable error message.  
...
#### OrderCancelRequestAndNewOrderSingle `<XCN>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordercancelrequestandnewordersingle-xcn "ordercancelrequestandnewordersingle-xcn的直接链接")
...
Please refer to [Supported Order Types](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordertype) for supported field combinations when describing the new order.
...
**Response:**
  ...
  * [ExecutionReport`<8>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#executionreport) with `ExecType (150)` value `CANCELED (4)` for the canceled order.
  * [ExecutionReport`<8>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#executionreport) with `ExecType (150)` value `NEW (0)` for the new order.
  * [ExecutionReport`<8>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#executionreport) with `ExecType (150)` value `REJECTED (8)` if the new order was rejected.
  * [OrderCancelReject`<9>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordercancelreject) if the cancellation was rejected.
  * [Reject`<3>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#reject) if the message is rejected.



#### OrderMassCancelRequest `<q>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordermasscancelrequest-q "ordermasscancelrequest-q的直接链接")
...
**Responses:**
  ...
  * [ExecutionReport`<8>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#executionreport) with `ExecType (150)` value `CANCELED (4)` for the every order canceled.
  * [OrderMassCancelReport`<r>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordermasscancelreport) with `MassCancelResponse (531)` field indicating whether the message is accepted or rejected.
  * [Reject`<3>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#reject) if the message is rejected.



#### OrderMassCancelReport `<r>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordermasscancelreport-r "ordermasscancelreport-r的直接链接")

Sent by the server in response to [OrderMassCancelRequest`<q>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordermasscancelrequest).
...
#### NewOrderList `<E>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#neworderlist-e "neworderlist-e的直接链接")
...
Orders in an order list are contingent on one another. Please refer to [Supported Order List Types](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#order-list-types) for supported order types and triggering instructions.
...
`2` \- ONE_TRIGGERS_THE_OTHER  
25046| OPO| BOOLEAN| N| Sets this order list as an [OPO](/docs/zh-CN/binance-spot-api-docs/faqs/opo.md) when set to `true`.  
73| NoOrders| NUMINGROUP| N| The length of the array for Orders. Only 2 or 3 are allowed.  
=>11| ClOrdID| STRING| Y| `ClOrdID` to be assigned to the order  
=>38| OrderQty| QTY| N| Quantity of the order  
=>40| OrdType| CHAR| Y| See the [table](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#ordertype) to understand supported order types and the required fields to use them.  
...
#### Supported Order List Types[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#supported-order-list-types "Supported Order List Types的直接链接")
...
#### ListStatus `<N>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#liststatus-n "liststatus-n的直接链接")

Sent by the server whenever an order list state changes.

> [!NOTE] By default, ListStatus`<N>` is sent for all order lists of an account, including those submitted in different connections. Please see [Response Mode](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#responsemode) for other behavior options.
...
#### OrderAmendKeepPriorityRequest `<XAK>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#orderamendkeeppriorityrequest-xak "orderamendkeeppriorityrequest-xak的直接链接")
...
Read [Order Amend Keep Priority FAQ](/docs/zh-CN/binance-spot-api-docs/faqs/order_amend_keep_priority.md) to learn more.
...
**Response:**
  ...
  * [Reject `<3>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#reject) if the incoming request is invalid either due to missing required fields, invalid fields, refers to an invalid symbol, or exceeds the message limit.
  * [OrderAmendReject `<XAR>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#orderamendreject) if failed due to insufficient order rate limits, pointing to a non-existent order, quantity is invalid, etc.
  * [ExecutionReport `<8>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#executionreport) if the request succeeded for amending a single order.
  * [ExecutionReport `<8>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#executionreport) \+ [ListStatus `<N>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#liststatus) if the request succeeded for amending an order which is part of an Order list.



### OrderAmendReject `<XAR>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#orderamendreject-xar "orderamendreject-xar的直接链接")
...
### Limit Messages[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#limit-messages "Limit Messages的直接链接")

#### LimitQuery `<XLQ>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#limitquery-xlq "limitquery-xlq的直接链接")
...
#### LimitResponse `<XLR>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#limitresponse-xlr "limitresponse-xlr的直接链接")

Sent by the server in response to [LimitQuery`<XLQ>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#limitquery).
...
### Market Data Messages[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#market-data-messages "Market Data Messages的直接链接")

> [!NOTE] The messages below can only be used for the FIX Market Data.

#### InstrumentListRequest `<x>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#instrumentlistrequest-x "instrumentlistrequest-x的直接链接")
...
#### InstrumentList `<y>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#instrumentlist-y "instrumentlist-y的直接链接")

Sent by the server in a response to the [InstrumentListRequest`<x>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#instrumentlistrequest).
...
=>55| Symbol| STRING| Y|   
=>15| Currency| STRING| Y| Quote asset of this symbol  
=>562| MinTradeVol| QTY| N| Corresponds to the [LOT_SIZE](/docs/zh-CN/binance-spot-api-docs/testnet/filters#lot_size) filter  
=>1140| MaxTradeVol| QTY| N| Corresponds to the [LOT_SIZE](/docs/zh-CN/binance-spot-api-docs/testnet/filters#lot_size) filter  
=>25039| MinQtyIncrement| QTY| N| Corresponds to the [LOT_SIZE](/docs/zh-CN/binance-spot-api-docs/testnet/filters#lot_size) filter  
=>25040| MarketMinTradeVol| QTY| N| Corresponds to the [MARKET_LOT_SIZE](/docs/zh-CN/binance-spot-api-docs/testnet/filters#market_lot_size) filter  
=>25041| MarketMaxTradeVol| QTY| N| Corresponds to the [MARKET_LOT_SIZE](/docs/zh-CN/binance-spot-api-docs/testnet/filters#market_lot_size) filter  
=>25042| MarketMinQtyIncrement| QTY| N| Corresponds to the [MARKET_LOT_SIZE](/docs/zh-CN/binance-spot-api-docs/testnet/filters#market_lot_size) filter  
=>969| MinPriceIncrement| PRICE| N| Corresponds to the [PRICE](/docs/zh-CN/binance-spot-api-docs/testnet/filters#price_filter) filter  
=>2551| StartPriceRange| PRICE| N| Corresponds to the [PRICE](/docs/zh-CN/binance-spot-api-docs/testnet/filters#price_filter) filter  
=>2552| EndPriceRange| PRICE| N| Corresponds to the [PRICE](/docs/zh-CN/binance-spot-api-docs/testnet/filters#price_filter) filter  
...
#### MarketDataRequest `<V>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#marketdatarequest-v "marketdatarequest-v的直接链接")
...
> [!NOTE] In the [Individual Symbol Book Ticker Stream](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#symbolbooktickerstream), when `MDUpdateAction` is set to `CHANGE(1)` in a [MarketDataIncrementalRefresh`<X>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#marketdataincrementalrefresh) message sent from the server, it replaces the previous best quote.
...
**Fields required to subscribe:**
  ...
  * `MDEntryType` with value `BID(0)`
  * `MDEntryType` with value `OFFER(1)`
  * `MarketDepth` with a value between `2` and `5000`, which controls the size of the initial snapshot and has no effect on subsequent [MarketDataIncrementalRefresh`<X>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#marketdataincrementalrefresh) messages



**Update Speed:** 100ms

> [!NOTE] Since the [MarketDataSnapshot`<W>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#marketdatasnapshot) have a limit on the number of price levels (5000 on each side maximum), you won't learn the quantities for the levels outside of the initial snapshot unless they change. So be careful when using the information for those levels, since they might not reflect the full view of the order book. However, for most use cases, seeing 5000 levels on each side is enough to understand the market and trade effectively.
...
### MarketDataRequestReject `<Y>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#marketdatarequestreject-y "marketdatarequestreject-y的直接链接")

Sent by the server in a response to an invalid MarketDataRequest `<V>`.

Tag| Name| Type| Required| Description  
---|---|---|---|---  
262| MDReqID| STRING| Y| ID of the invalid [MarketDataRequest`<V>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#marketdatarequest)  
281| MDReqRejReason| CHAR| N| Possible values:   
  
`1` \- DUPLICATE_MDREQID   
  
`2` \- TOO_MANY_SUBSCRIPTIONS  
25016| ErrorCode| INT| N| API Error code. See [Errors](/docs/zh-CN/binance-spot-api-docs/testnet/errors)  
58| Text| STRING| N| Human-readable error message.  
...
### MarketDataSnapshot `<W>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#marketdatasnapshot-w "marketdatasnapshot-w的直接链接")

Sent by the server in response to a [MarketDataRequest`<V>`](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#marketdatarequest), activating [Individual Symbol Book Ticker Stream](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#symbolbooktickerstream) or [Diff. Depth Stream](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#diffdepthstream) subscriptions.
...
### MarketDataIncrementalRefresh `<X>`[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#marketdataincrementalrefresh-x "marketdataincrementalrefresh-x的直接链接")
...
`2` \- SELL  
=>25043| FirstBookUpdateID| INT| N| Only present in [Diff. Depth Stream](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#diffdepthstream).   
  
Market Data Entry will default to the same `FirstBookUpdateID` of the previous Market Data Entry in the same Market Data message if `FirstBookUpdateID` is not specified  
=>25044| LastBookUpdateID| INT| N| Only present in [Diff. Depth Stream](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#diffdepthstream) and [Individual Symbol Book Ticker Stream](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#symbolbooktickerstream).   
...
### SBE[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#sbe "SBE的直接链接")

Read the [SBE FAQ](/docs/zh-CN/binance-spot-api-docs/faqs/sbe_faq.md) for important information about using SBE with Binance APIs.
...
### Endpoints[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#endpoints "Endpoints的直接链接")
...
### FIX SBE encoding layout[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#fix-sbe-encoding-layout "FIX SBE encoding layout的直接链接")
...
### Logon[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#logon "Logon的直接链接")

The logon signature (RawData) is computed as documented in the [signature computation](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#signaturecomputation) section.

#### Sample FIX SBE `Logon` request message[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#sample-fix-sbe-logon-request-message "sample-fix-sbe-logon-request-message的直接链接")
...
### Limits[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#limits-1 "Limits的直接链接")

Connection limits are shared between FIX and FIX SBE.

### Errors[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#errors "Errors的直接链接")
...
### FAQ[​](/docs/zh-CN/binance-spot-api-docs/testnet/fix-api#faq "FAQ的直接链接")

See the [SBE FAQ](/docs/zh-CN/binance-spot-api-docs/faqs/sbe_faq.md) for more information on generating SBE decoders and handling schema updates.