...
api_type: Account
updated_at: 2026-01-15T23:51:21.436003
---

# General Info

## General API Information[​](/docs/sub_account/general-info#general-api-information "Direct link to General API Information")
...
### HTTP Return Codes[​](/docs/sub_account/general-info#http-return-codes "Direct link to HTTP Return Codes")
...
### Error Codes and Messages[​](/docs/sub_account/general-info#error-codes-and-messages "Direct link to Error Codes and Messages")
...
> The error payload on API and SAPI is as follows:
  ...
  * Specific error codes and messages defined in [Error Codes](/docs/sub_account/general-info#error-codes).



### General Information on Endpoints[​](/docs/sub_account/general-info#general-information-on-endpoints "Direct link to General Information on Endpoints")
...
## LIMITS[​](/docs/sub_account/general-info#limits "Direct link to LIMITS")

### General Info on Limits[​](/docs/sub_account/general-info#general-info-on-limits "Direct link to General Info on Limits")
...
### IP Limits[​](/docs/sub_account/general-info#ip-limits "Direct link to IP Limits")
...
### Order Rate Limits[​](/docs/sub_account/general-info#order-rate-limits "Direct link to Order Rate Limits")
...
### Websocket Limits[​](/docs/sub_account/general-info#websocket-limits "Direct link to Websocket Limits")
...
### /api/ and /sapi/ Limit Introduction[​](/docs/sub_account/general-info#api-and-sapi-limit-introduction "Direct link to /api/ and /sapi/ Limit Introduction")
...
## Data Sources[​](/docs/sub_account/general-info#data-sources "Direct link to Data Sources")
...
## Request Security[​](/docs/sub_account/general-info#request-security "Direct link to Request Security")

  * Each endpoint has a security type indicating required API key permissions, shown next to the endpoint name (e.g., [New order (TRADE)](/docs/sub_account/general-info#place-new-order-trade)).
  * If unspecified, the security type is `NONE`.
  * Except for `NONE`, all endpoints with a security type are considered `SIGNED` requests (i.e. including a `signature`), except for [listenKey management](/docs/sub_account/general-info#user-data-stream-requests).
...
### SIGNED Endpoint security[​](/docs/sub_account/general-info#signed-endpoint-security "Direct link to SIGNED Endpoint security")

  * `SIGNED` endpoints require an additional parameter, `signature`, to be sent in the `query string` or `request body`.



#### Signature Case Sensitivity[​](/docs/sub_account/general-info#signature-case-sensitivity "Direct link to Signature Case Sensitivity")
...
Please consult [SIGNED request example (HMAC)](/docs/sub_account/general-info#hmac-keys), [SIGNED request example (RSA)](/docs/sub_account/general-info#rsa-keys), and [SIGNED request example (Ed25519)](/docs/sub_account/general-info#ed25519-keys) on how to compute signature, depending on which API key type you are using.

### Timing security[​](/docs/sub_account/general-info#timing-security "Direct link to Timing security")

  * `SIGNED` requests also require a `timestamp` parameter which should be the current timestamp either in milliseconds or microseconds. (See [General API Information](/docs/sub_account/general-info#general-api-information))
...
### SIGNED Endpoint Examples for POST /api/v3/order[​](/docs/sub_account/general-info#signed-endpoint-examples-for-post-apiv3order "Direct link to SIGNED Endpoint Examples for POST /api/v3/order")

#### HMAC Keys[​](/docs/sub_account/general-info#hmac-keys "Direct link to HMAC Keys")
...
#### RSA Keys[​](/docs/sub_account/general-info#rsa-keys "Direct link to RSA Keys")
...
#### Ed25519 Keys[​](/docs/sub_account/general-info#ed25519-keys "Direct link to Ed25519 Keys")
...
## API 基本信息[​](/docs/zh-CN/sub_account/general-info#api-基本信息 "API 基本信息的直接链接")
...
### HTTP 返回代码[​](/docs/zh-CN/sub_account/general-info#http-返回代码 "HTTP 返回代码的直接链接")
...
### 接口错误代码[​](/docs/zh-CN/sub_account/general-info#接口错误代码 "接口错误代码的直接链接")
...
> API 与 SAPI 的错误代码返回形式如下:
  ...
  * 具体的错误码及其解释在 [错误代码](/docs/zh-CN/sub_account/general-info#cf68bca02a).



### 接口的基本信息[​](/docs/zh-CN/sub_account/general-info#接口的基本信息 "接口的基本信息的直接链接")
...
## 访问限制[​](/docs/zh-CN/sub_account/general-info#访问限制 "访问限制的直接链接")

### 访问限制基本信息[​](/docs/zh-CN/sub_account/general-info#访问限制基本信息 "访问限制基本信息的直接链接")
...
### IP 访问限制[​](/docs/zh-CN/sub_account/general-info#ip-访问限制 "IP 访问限制的直接链接")
...
### WEB SOCKET 连接限制[​](/docs/zh-CN/sub_account/general-info#web-socket-连接限制 "WEB SOCKET 连接限制的直接链接")
...
### /api/ 与 /sapi/ 接口限频说明[​](/docs/zh-CN/sub_account/general-info#api-与-sapi-接口限频说明 "/api/ 与 /sapi/ 接口限频说明的直接链接")
...
## 数据来源[​](/docs/zh-CN/sub_account/general-info#数据来源 "数据来源的直接链接")
...
## 请求鉴权类型[​](/docs/zh-CN/sub_account/general-info#请求鉴权类型 "请求鉴权类型的直接链接")

  * 每个接口都有一个鉴权类型，指示所需的 API 密钥权限，显示在接口名称旁边（例如，[下新订单 (TRADE)](/docs/zh-CN/sub_account/general-info#place-new-order-trade)）。
  * 如果未指定，则鉴权类型为 `NONE`。
  * 除了为 `NONE` 外，所有具有鉴权类型的接口均视为 `SIGNED` 请求（即包含 `signature`），[listenKey 管理](/docs/zh-CN/sub_account/general-info#user-data-stream-requests) 除外。
...
### 需要签名的接口[​](/docs/zh-CN/sub_account/general-info#需要签名的接口 "需要签名的接口的直接链接")

  * 调用`SIGNED` 接口时，除了接口本身所需的参数外，还需要在 `query string` 或 `request body` 中传递 `signature`, 即签名参数。



#### 签名是否是大小写敏感的[​](/docs/zh-CN/sub_account/general-info#签名是否是大小写敏感的 "签名是否是大小写敏感的的直接链接")
...
请参阅[已签名请求示例 (HMAC)](/docs/zh-CN/sub_account/general-info#hmac-keys)、[已签名请求示例 (RSA)](/docs/zh-CN/sub_account/general-info#rsa-keys) 和[已签名请求示例 (Ed25519)](/docs/zh-CN/sub_account/general-info#ed25519-keys)，了解如何根据您使用的 API 密钥类型计算签名。

### 时间同步安全[​](/docs/zh-CN/sub_account/general-info#时间同步安全 "时间同步安全的直接链接")

  * `SIGNED` 请求还需要一个 `timestamp` 参数，该参数应为当前时间戳，单位为毫秒或微秒。（参见 [通用 API 信息](/docs/zh-CN/sub_account/general-info#general-api-information)）
...
### POST /api/v3/order 的签名示例[​](/docs/zh-CN/sub_account/general-info#post-apiv3order-的签名示例 "POST /api/v3/order 的签名示例的直接链接")

#### HMAC Keys[​](/docs/zh-CN/sub_account/general-info#hmac-keys "HMAC Keys的直接链接")
...
#### RSA Keys[​](/docs/zh-CN/sub_account/general-info#rsa-keys "RSA Keys的直接链接")
...
#### Ed25519 Keys[​](/docs/zh-CN/sub_account/general-info#ed25519-keys "Ed25519 Keys的直接链接")
